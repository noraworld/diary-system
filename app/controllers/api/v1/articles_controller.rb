...
class Api::V1::ArticlesController < ApplicationController
  ...
  def index
    year = params[:year].to_i.zero? ? adjusted_current_time.strftime('%Y').to_i : params[:year].to_i
    month = params[:month].to_i.zero? ? adjusted_current_time.strftime('%m').to_i : params[:month].to_i
    articles = Article.order('day').where(year: year).where(month: month)
    render json: articles, adjusted_current_time: adjusted_current_time
  end
