...
class MigrateForm
  ...
  validates :from, format: { with: /\A[0-9]{4}-[0-9]{2}-[0-9]{2}\z/, message: 'The "from" parameter is out of range of date type' }, allow_nil: true

  validates :to, presence: { message: 'The destination date ("to" parameter) is missing.' }
  validates :to, format: { with: /\A[0-9]{4}-[0-9]{2}-[0-9]{2}\z/, message: 'The "to" parameter is out of range of date type' }, allow_nil: true
  ...
  def check_origin_existence
    return if from&.match(/\A[0-9]{4}-[0-9]{2}-[0-9]{2}\z/).nil?
  ...
  def check_destination_existence
    return if to&.match(/\A[0-9]{4}-[0-9]{2}-[0-9]{2}\z/).nil?
